__doc__ = """
            INPUT：SRR_list
            PROCESS：利用sratoolkit、STAR、featureCounts处理SRR文件
            """
...
def first_work(task_file):
    ...
    for file in os.listdir(organism_genome):
        ...
        print('未找到物种%s的gtf文件：%s' % (organism, organism_genome))
        exit()
    # ====================定位到软件的位置====================
    star = STAR
...
def srr_handle(srr):
    ...
    if not os.path.exists(srr_file):
        return 1, '在data文件夹未找到%s' % srr_file
...
def main_work(task_file):
    ...
    for count, srr in enumerate(task_list, 1):
        ...
        text = '开始处理：%s  —  %s  大小：%sG (%d/%d)' % (organism, srr, str(file_size), count, len(task_list))
        print(add_color(text, 'yellow'))

        foo, command = srr_handle(srr)
        if foo:
            text = '处理错误：%s —  %s\n 命令：%s' % (organism, srr, command)
            print(add_color(text, 'red'))
            error.append(srr)
        else:
            text = '处理成功：%s —  %s' % (organism, srr)
    ...
    # ====================输出这个进程处理的报告====================
    text = '任务：%s处理完了' % task_file
    print(add_color(text, 'green'))

    if error:
        text = '%s 没有正确处理' % str(error)
        print(add_color(text, 'red'))
    else:
        text = '所有SRR都处理完全'
        print(add_color(text, 'green'))
